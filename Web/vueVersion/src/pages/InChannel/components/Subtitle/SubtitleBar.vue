    ...
    <template v-if="sortedSpeakers.length">
      <Row class="subTitleList">
        <Row v-for="uid in sortedSpeakers" class="subTitleItem">
          ...
          <Col v-if="messageMap[uid]?.origin && (asrInfo.transLang === 'source' || asrInfo.dualLang)"
            class="subTitleSource">{{ messageMap[uid]?.origin }}</Col>
          <Col v-if="messageMap[uid]?.trans && asrInfo.transLang !== 'source'"
            :class="[asrInfo.dualLang ? 'subTitleTranslated' : 'subTitleSource']">{{ messageMap[uid]?.trans }}</Col>
        </Row>
      </Row>
    </template>
    <template v-else>
