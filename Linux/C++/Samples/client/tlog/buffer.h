...
struct BufferCompat {
    static constexpr bool value =
        !std::is_volatile<U>::value && ((std::is_integral<T>::value && sizeof(T) == 1)
...
public:
    ...
    BufferT(U* data, size_t size, size_t capacity)
        ...
        memcpy(data_.get(), data, size * sizeof(U));
    }
    ...
    bool operator==(const BufferT& buf) const {
        ...
        if (std::is_integral<T>::value) {
            // Optimization.
            return memcmp(data_.get(), buf.data_.get(), size_ * sizeof(T)) == 0;
        }
    ...
    void AppendData(const U* data, size_t size) {
        ...
        static_assert(sizeof(T) == sizeof(U), "");
        memcpy(data_.get() + size_, data, size * sizeof(U));
        size_ = new_size;
        // RTC_DCHECK(IsConsistent());
    }
