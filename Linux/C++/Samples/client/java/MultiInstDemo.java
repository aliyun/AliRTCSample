...
/*
 *  demo run : ./run_multi_inst_demo.sh
 */
public class MultiInstDemo {
    ...
    public static void main(String[] args) {
        ...
        int count = 10;
        final CountDownLatch barrier = new CountDownLatch(count);
                    ...
                    public void run() {
                        ...
                        if (parseAppConfig(context, appConfigPath)) {
                            if (!context.manualControl) {
                                ...
                                joinAndLeaveChannel(context, barrier, authInfo);
                            }
                        }
                    }
                }).start();
        ...
        if (parseAppConfig(context, appConfigPath)) {
            ...
            if (!context.manualControl) {
                ...
                for (int i = 0; i < joinLeaveCount; i++) {
                    joinAndLeaveChannel(context, null, authInfo);
                }
            } else {
    ...
    private static DingRtcEngine initRtcEngine(RtcContext context) {
        ...
        // }
        // DingRtcEngine.setLogDirPath(dir);
        DingRtcEngine engine = DingRtcEngine.createInstance(context.listener, "");
    ...
    private static void joinAndLeaveChannel(RtcContext context, CountDownLatch barrier, DingRtcEngine.DingRtcAuthInfo authInfo) {
    ...
    private static DingRtcEngine.DingRtcAuthInfo getAuthInfo(RtcContext context) {
        ...
        String roomId = context.channelName + String.valueOf(i++);
