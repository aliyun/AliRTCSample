  ...
  for (c = ifp->mgr->conns; c != NULL; c = c->next) {
    ...
    if (now > s->timer) {
      if (s->ttype == MIP_TTYPE_ACK && s->acked != s->ack) {
        ...
        MG_INFO(("mg_tcpip_poll is_closing, MIP_TTYPE_FIN"));
        c->is_closing = 1;
        continue;
      } else {
        if (s->tmiss++ > 2) {
...
static void iolog(struct mg_connection *c, char *buf, long n, bool r) {
  if (n == MG_IO_WAIT) {
    ...
    MG_INFO(("iolog is_closing, n: %ld", n));
  ...
  if (ioalloc(c, &c->recv)) {
    ...
    if (c->is_tls) {
      ...
      if (n == MG_IO_ERR && c->rtls.len == 0) {
        ...
        MG_INFO(("read_conn is_closing, MG_IO_ERR and rtls.len 0"));
        c->is_closing = 1;
      } else {
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  for (c = mgr->conns; c != NULL; c = tmp) {
    ...
    if (c->is_draining && c->send.len == 0) {
      MG_INFO(("mg_mgr_poll is_closing, is_draining and send len 0"));
      c->is_closing = 1;
    }
    if (c->is_closing) close_conn(c);
  }
